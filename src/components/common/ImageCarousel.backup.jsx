...
const ImageCarousel = ({ 
  ...
  const loadImage = useCallback((index) => {
    if (!limitedImages[index]) return
    
    // Verificar si ya está cargada usando setLoadedImages con función callback
    setLoadedImages(prev => {
      // Si ya está cargada, no hacer nada
      if (prev.has(index)) return prev
      
      // Si no está cargada, iniciar la carga
      const img = new Image()
      const imageUrl = limitedImages[index].thumbnail_url || limitedImages[index].url || limitedImages[index]
      
      img.onload = () => {
        setLoadedImages(current => new Set([...current, index]))
      }
      img.onerror = () => {
      ...
      // Retornar el estado actual sin cambios (la actualización real ocurre en img.onload)
      return prev
    })
  }, [limitedImages])
      ...
      <div className="relative flex items-center justify-center">
        {isVisible && loadedImages.has(currentIndex) ? (
          <img
            ...
            className="w-full h-full transition-opacity duration-300 object-cover aspect-auto"
            onClick={() => onImageClick?.(currentIndex)}
          />
        ) : (
          <div className="w-full h-full bg-gray-200 flex items-center justify-center aspect-auto">
            <div className="text-gray-500 text-center p-8">
              <p>Loading image...</p>
            </div>
          </div>
